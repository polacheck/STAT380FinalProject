...
Next, we wanted to write a function that went through the data and calculated the winning percentage.  We did this, by writing one that looked at three pieces of data, determined if it was NA or not.  If it was, the count went up and we included the percentage in the total.  If not, it was not included.
```{r}
...
ggplot(aes(x = Win_Percent, y = Total_Score)) +
  ...
  ggtitle("NFL Winning Percentage vs Happiness Score")
```
...
Our bootstrapped seemed to appear mostly normal.  This means we can use it a lot easier for further analysis
```{r}
BootStrapTrials %>%
  ggplot() + 
  geom_histogram(aes(x = quantile))
...
Then, changing paths slightly, I tried to apply a linear model to happiness and winning percentage. We just used the data from 2018 as our data set. The data gives us a almost horizontal line at .544.  This shows that initially it is guessing there is almost no correlation between the two.  I had hoped to maybe correlate happiness to playoff aspirations, but as there is not a strong relationship between happiness and winning percentage, I don't think that relationship would exist.
...
After analyzing the analysis, it estimates the R squared value to be -.03422.  This means that there is probably no correlation between NFL team success and happiness.
```{r}
msummary(sim1_lm)
```
...
Then, as the happiness seemed to be unrelated from winning percentage, I built a simulation to estimate if a NFL team will make the playoffs.  This assumes each team has a random chance each year to make playoffs.  For this one, I just did a uniform distribution, ran it 10,000 times, and calculated the times it was above .625 (a 10-6 record - almost always enough to get a team into playoffs).
...
sim_meet <- data.frame(
  ...
  mutate(result = ifelse(you >= .625, 
                         "You Made Playoffs!", "Guess it was a rebuildling year"))
tally(~ result, format = "percent", data = sim_meet)
```

Then, I made another simulation, changing the number of simulations we ran.  This shows that as we took more and more trials, the data got more and more normal.
...
Then, I ran a simulation, trying to find when a city was happy and had a playoff team.  I assumed a normal distribution for simulation's sake and wanted to find out when a city was happy and had a strong NFL team.
```{r}
n <- 100000
